...
class AudioManager: NSObject {
  ...
  // Track original input gain to restore it later
  private var originalInputGain: Float = 1.0
  private var isPlayingAudio: Bool = false
  private let inputGainDuringPlayback: Float = 0.05 // Reduce mic to 15% during playback
  ...
  private func duckMicrophone() {
//    guard !isPlayingAudio else { return } // Already ducked
    
    do {
      let audioSession = AVAudioSession.sharedInstance()
      
      // Store current gain before ducking
      originalInputGain = audioSession.inputGain
      
      // Check if input gain is settable on this device
      if audioSession.isInputGainSettable {
        try audioSession.setInputGain(inputGainDuringPlayback)
        CoreCommsService.log("AudioManager: Ducked microphone input to \(inputGainDuringPlayback)")
      } else {
        CoreCommsService.log("AudioManager: Input gain is not settable on this device")
      }
      
//      isPlayingAudio = true
    } catch {
      CoreCommsService.log("AudioManager: Failed to duck microphone: \(error)")
    }
  }
  
  private func restoreMicrophone() {
    guard isPlayingAudio else { return } // Not ducked
    
    // Check if any audio is still playing
    if !players.isEmpty || !streamingPlayers.isEmpty {
      CoreCommsService.log("AudioManager: Audio still playing, keeping microphone ducked")
      return
    }
    ...
    do {
      ...
      if audioSession.isInputGainSettable {
        try audioSession.setInputGain(originalInputGain)
        CoreCommsService.log("AudioManager: Restored microphone input to \(originalInputGain)")
      }
      
      isPlayingAudio = false
    } catch {
      CoreCommsService.log("AudioManager: Failed to restore microphone: \(error)")
    }
  }
  ...
  private func playAudioFromUrl(requestId: String, url: String, volume: Float) {
    ...
    let endObserver = NotificationCenter.default.addObserver(
      ...
      // Restore microphone after playback ends
      self?.restoreMicrophone()
    }
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
      ...
      // Restore microphone after loading failure
      self?.restoreMicrophone()
    }
  ...
  func stopAudio(requestId: String) {
    ...
    // Restore microphone if no other audio is playing
    restoreMicrophone()
