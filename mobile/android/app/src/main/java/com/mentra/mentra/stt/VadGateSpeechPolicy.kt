...
class VadGateSpeechPolicy(private val context: Context) {
    companion object {
        ...
        private const val LONG_SILENCE_VAD_INTERVAL_MS = 200L    // Less frequent after extended silence

        // Thresholds for switching intervals
        private const val MEDIUM_SILENCE_THRESHOLD_MS = 20000L   // Switch to medium interval after 20s
    ...
    private fun startVad() {
        try {
            // Set internal silence duration very low; external logic manages the full required silence duration
            vad = Vad.builder()
                .setContext(context)
    ...
    fun processAudioBytes(bytes: ByteArray, offset: Int, length: Int) {
        ...
        // If in speech state and it hasn't been 8 seconds since the last speech was detected, skip processing
        ...
        if (totalSamples % FRAME_SIZE != 0) {
            Log.e(TAG, "Invalid audio frame size: $totalSamples samples. Needs to be multiple of $FRAME_SIZE.")
            return
        }
        ...
        for (i in 0 until numFrames) {
            ...
            if (detectedSpeech) {
                ...
                // If no speech detected, and 8 seconds have elapsed since the last speech, mark as silence
            ...
            if (isCurrentlySpeech != previousSpeechState) {
                Log.d(TAG, "Speech detection changed to: ${if (isCurrentlySpeech) "SPEECH" else "SILENCE"}")
