...
export const reportError = (
  ...
  const report = createReport().message(message).level(ReportLevel.ERROR).category(category).operation(operation)
...
export const reportWarning = (
  ...
  const report = createReport().message(message).level(ReportLevel.WARNING).category(category).operation(operation)
...
export const reportInfo = (message: string, category: string, operation: string, tags?: Record<string, any>): void => {
  const report = createReport().message(message).level(ReportLevel.INFO).category(category).operation(operation)
...
export const reportCritical = (
  ...
  const report = createReport().message(message).level(ReportLevel.CRITICAL).category(category).operation(operation)
