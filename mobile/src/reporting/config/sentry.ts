...
export const getSentryDsn = (): string | undefined => {
  ...
  const dsn = process.env[SENTRY_DSN_KEY] || process.env.EXPO_PUBLIC_SENTRY_DSN || undefined
...
export const getSentryOrg = (): string | undefined => {
  return process.env[SENTRY_ORG_KEY] || process.env.EXPO_PUBLIC_SENTRY_ORG || undefined
}
...
export const getSentryProject = (): string | undefined => {
  return process.env[SENTRY_PROJECT_KEY] || process.env.EXPO_PUBLIC_SENTRY_PROJECT || undefined
}
...
export const getSentryUrl = (): string => {
  return process.env[SENTRY_URL_KEY] || process.env.EXPO_PUBLIC_SENTRY_URL || "https://sentry.io/"
}
