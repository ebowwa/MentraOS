...
import MetaGlassesMirror from "@/components/mirror/MetaGlassesMirror"
...
export const CompactDeviceStatus = ({ style }: { style?: ViewStyle }) => {
  ...
  if (showSimulatedGlasses) {
    // Determine which mirror component to show based on glasses type
    const isMetaGlasses = defaultWearable === DeviceTypes.META_RAYBAN
    ...
    return (
      <View style={[themed($container), style]}>
        ...
        <View style={{ marginHorizontal: -theme.spacing.s6 }}>
          {isMetaGlasses ? (
            <MetaGlassesMirror style={{ marginHorizontal: theme.spacing.s6 }} />
          ) : (
