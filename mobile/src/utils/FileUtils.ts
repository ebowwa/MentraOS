      ...
      if (FileProviderModule) {
        ...
        if (FileProviderModule.shareFile) {
          ...
          try {
            ...
            const pathForNative = normalizedPath.startsWith("file://") ? normalizedPath.substring(7) : normalizedPath

            console.log("Path for native module:", pathForNative)

            // Use the direct native sharing method
            await FileProviderModule.shareFile(pathForNative, mimeType, title || "Share File", message || "")
            ...
            reportDirectFileSharingFailure(
              filePath,
              String(directShareError),
              directShareError instanceof Error ? directShareError : new Error(String(directShareError)),
            )
        ...
        try {
          ...
          reportContentUriRetrievalFailure(
            filePath,
            String(uriError),
            uriError instanceof Error ? uriError : new Error(String(uriError)),
          )
