  ...
  private async handleGlassesMessage(
    ...
    try {
      ...
      switch (message.type) {
        ...
        case GlassesToCloudMessageType.VIDEO_FRAME:
          const frameMsg = message as any;
          userSession.logger.info(
            { service: SERVICE_NAME, frameSize: frameMsg.base64?.length || 0 },
            "📹 Received video frame from Meta glasses (timestamp: " + frameMsg.frameTimestamp + ", quality: " + frameMsg.quality + ")"
          );
          // TODO: Process video frame - relay to LiveKit, save to storage, etc.
          break;
