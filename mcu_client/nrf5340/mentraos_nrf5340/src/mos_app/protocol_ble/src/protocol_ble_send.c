 ...
 * @LastEditTime : 2025-08-20 09:42:22
...
void handler_image_transfer_complete(const ble_protocol_msg_t *msg)
{
    ...
    if ((root == NULL) || (!msg))
    {
        LOG_ERR("Failed to create JSON object || msg is NULL");
    }
    ...
    {
        // LOG_INF("JSON missing_chunks!!!");
        // LOG_HEXDUMP_INF(msg->data.image_transfer_complete.missing_chunks, msg->data.image_transfer_complete.missing_count, "Hexdump");
    ...
    if (json == NULL)
    {
        LOG_INF("cJSON_PrintUnformatted failed");
        cJSON_Delete(root);
        return;
    }
...
void ble_protocol_send_init(void *p1, void *p2, void *p3)
{
    ...
    LOG_INF("cJSON version: %s", cJSON_Version());
