 ...
 * @LastEditTime : 2025-08-20 09:30:54
...
#define LOG_MODULE_NAME BSPAL_ICM42688P
...
{
    uint8_t buf[12];
    int     ret = i2c_burst_read(i2c_dev_icm42688p, ICM42688P_I2C_ADDR, REG_ACCEL_DATA_X1, buf, sizeof(buf));
    if (ret)
    {
...
void bspal_icm42688p_parameter_config(void)
{
    ...
    mos_delay_ms(50);  // 陀螺仪切换到 LN 模式需要 ≥45 ms 稳定 ；The gyroscope needs to be stable for at least 45 ms
                       // after switching to LN mode
...
void test_icm42688p(void)
{
    ...
    if (icm_read_motion(acc_raw, gyr_raw) == 0)
    {
        ...
        // Convert to physical quantity by ±16g/±2000dps
        for (int i = 0; i < 3; i++)
        {
            icm42688p_data.acc_g[i]   = acc_raw[i] * (16.0f / 32768.0f);     // unit：g
            icm42688p_data.acc_ms2[i] = icm42688p_data.acc_g[i] * 9.80665f;  // unit：m/s²
            icm42688p_data.gyr_dps[i] = gyr_raw[i] * (2000.0f / 32768.0f);   // unit：deg/s
        }
        ...
        LOG_INF("GYR: X=%.4f dps  Y=%.4f dps  Z=%.4f dps", icm42688p_data.gyr_dps[0], icm42688p_data.gyr_dps[1],
