                        ...
                        uint32_t time_ms)
{
    ...
    // 强制标签布局更新，获取正确的内容高度
    lv_obj_update_layout(label); // Force label layout update to get correct content height
...
static void show_default_ui(void)
{
    ...
    //                    12000); // 往返周期 4000ms; ping-pong cycle 4000ms
    // LV_IMG_DECLARE(my_img);   // 由 LVGL 的 img converter 工具生成 C 数组; generated C array by LVGL's img converter tool
...
void lvgl_dispaly_init(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        ...
        /* 处理完命令后，立即再刷一次画面，保证命令效果能马上显现;
         * After processing the command, immediately refresh the screen to ensure the command effect is displayed immediately */
        if (state_type == LCD_STATE_ON)
        {
            lv_timer_handler();
        }
