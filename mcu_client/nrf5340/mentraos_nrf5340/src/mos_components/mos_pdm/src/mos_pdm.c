...
static uint8_t pdm_fill_idx = 0;  // 下次填充给 PDM 的 hw buf 索引

static volatile uint8_t fifo_head = 0;  // 写指针（中断用）
static volatile uint8_t fifo_tail = 0;  // 读指针（上层用）
...
static void             pcm_buffer_req_evt_handle(const nrfx_pdm_evt_t *evt)
{
    ...
    {
        const int16_t *done = evt->buffer_released;  // 直接就是数据首地址
        (void)fifo_push(done);
        mos_sem_give(&pcmsem);
    }
...
uint32_t get_pdm_sample(int16_t *pdm_pcm_data, uint32_t pdm_pcm_szie)
{
    if (pdm_pcm_data == NULL || pdm_pcm_szie < PDM_PCM_REQ_BUFFER_SIZE)
    {
        ...
        return MOS_OS_ERROR;  // 目标缓冲太小或空指针
    }
