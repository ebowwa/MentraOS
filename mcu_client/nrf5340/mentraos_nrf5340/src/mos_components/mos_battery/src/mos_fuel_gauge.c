 ...
 * @LastEditTime : 2025-08-19 14:13:56
...
int fuel_gauge_update(const struct device *charger, bool vbus_connected)
{
	...
	soc = nrf_fuel_gauge_process(voltage, current, temp, delta, NULL); // v -测量的电池电压[V]。 i -测量的电池电流[A]。 T -测量的电池温度[C]；v - Measured battery voltage [V]. i - Measured battery current [A]. T - Measured battery temperature [C];
	tte = nrf_fuel_gauge_tte_get();									   // 获取电池在当前放电条件下预计还能使用的时间； Get the time the battery can still be used under the current discharge conditions;
	ttf = nrf_fuel_gauge_ttf_get();									   // 获取电池在当前充电条件下预计还需多长时间才能充满； Get how long it will take to fully charge the battery under the current charging conditions;
