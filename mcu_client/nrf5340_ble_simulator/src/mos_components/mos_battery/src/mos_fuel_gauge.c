 ...
 * @LastEditTime : 2025-09-08 16:29:43
...
static int read_sensors(const struct device *charger, float *voltage, float *current, float *temp, int32_t *chg_status)
{
    ...
    // 从传感器中获取数据;Fetch data from the sensor
    ...
    // 从传感器中获取电压值;Get voltage value from the sensor
    sensor_channel_get(charger, SENSOR_CHAN_GAUGE_VOLTAGE, &value);
    // 将传感器值转换为浮点数; Convert the sensor value to a float
    *voltage = (float)value.val1 + ((float)value.val2 / 1000000);

    // 从传感器中获取温度值;Get temperature value from the sensor
    sensor_channel_get(charger, SENSOR_CHAN_GAUGE_TEMP, &value);
    // 将传感器值转换为浮点数; Convert the sensor value to a float
    *temp = (float)value.val1 + ((float)value.val2 / 1000000);

    // 从传感器中获取电流值;Get current value from the sensor
    sensor_channel_get(charger, SENSOR_CHAN_GAUGE_AVG_CURRENT, &value);
    // 将传感器值转换为浮点数; Convert the sensor value to a float
    *current = (float)value.val1 + ((float)value.val2 / 1000000);

    // 从传感器中获取充电状态值;Get charge status value from the sensor
    sensor_channel_get(charger, SENSOR_CHAN_NPM1300_CHARGER_STATUS, &value);
    // 将传感器值转换为整型; Convert the sensor value to an integer
    *chg_status = value.val1;
    return 0;
}

// 静态函数，用于通知充电状态; Static function for notifying charge status
static int charge_status_inform(int32_t chg_status)
{
    union nrf_fuel_gauge_ext_state_info_data state_info;

    // 如果充电状态包含完成标志; If the charge status contains the complete flag
    if (chg_status & NPM1300_CHG_STATUS_COMPLETE_MASK)
    {
		// 打印充电完成信息; Print charge complete information
        BSP_LOGI(TAG, "Charge complete");
        // 设置充电状态为完成; Set charge state to complete
        state_info.charge_state = NRF_FUEL_GAUGE_CHARGE_STATE_COMPLETE;
    }
    // 如果充电状态包含涓流充电标志; If the charge status contains the trickle charge flag
    else if (chg_status & NPM1300_CHG_STATUS_TRICKLE_MASK)
    {
		// 打印涓流充电信息; Print trickle charge information
        BSP_LOGI(TAG, "Trickle charging");
        // 设置充电状态为涓流充电; Set charge state to trickle charge
        state_info.charge_state = NRF_FUEL_GAUGE_CHARGE_STATE_TRICKLE;
    }
    // 如果充电状态包含恒流充电标志; If the charge status contains the constant current charge flag
    else if (chg_status & NPM1300_CHG_STATUS_CC_MASK)
    {
        // 打印恒流充电信息; Print constant current charge information
        BSP_LOGI(TAG, "Constant current charging");
        // 设置充电状态为恒流充电; Set charge state to constant current charge
        state_info.charge_state = NRF_FUEL_GAUGE_CHARGE_STATE_CC;
    }
    // 如果充电状态包含恒压充电标志; If the charge status contains the constant voltage charge flag
    else if (chg_status & NPM1300_CHG_STATUS_CV_MASK)
    {
        // 打印恒压充电信息; Print constant voltage charge information
        BSP_LOGI(TAG, "Constant voltage charging");
        // 设置充电状态为恒压充电; Set charge state to constant voltage charge
        state_info.charge_state = NRF_FUEL_GAUGE_CHARGE_STATE_CV;
    }
    else // 否则，充电状态为空闲; Otherwise, the charge state is idle
    {
        // 打印充电空闲信息; Print charger idle information
        BSP_LOGI(TAG, "Charger idle");
        // 设置充电状态为空闲; Set charge state to idle
        state_info.charge_state = NRF_FUEL_GAUGE_CHARGE_STATE_IDLE;
    }

    // 更新充电状态信息; Update charge state information
...
{
    struct sensor_value value;// 用于存储传感器值的变量; Variable for storing sensor values
    ...
    // 读取传感器数据，并存储到parameters中; Read sensor data and store it in parameters
    ...
    // 计算终止充电电流，并存储到term_charge_current中; Calculate termination charge current and store it in term_charge_current
    term_charge_current = max_charge_current / 10.f;

    // 初始化nRF Fuel Gauge; Initialize nRF Fuel Gauge
    ...
    // 设置nRF Fuel Gauge的充电电流限制; Set charge current limit for nRF Fuel Gauge
    ...
    // 设置nRF Fuel Gauge的终止充电电流; Set termination charge current for nRF Fuel Gauge
    ret = nrf_fuel_gauge_ext_state_update(
...
int fuel_gauge_update(const struct device *charger, bool vbus_connected)
{
    ...
    // 从充电器设备读取电压、电流、温度、充电状态等信息; Read voltage, current, temperature, charge status, etc. from the charger device
    ...
    // 通知充电器设备当前的状态; Notify the charger device of the current state
    ret = nrf_fuel_gauge_ext_state_update(
    ...
    // 如果充电状态发生了变化; If the charge state has changed
    if (chg_status != chg_status_prev)
    {
        // 更新上一次的充电状态; Update the previous charge state
        chg_status_prev = chg_status;

        // 通知充电状态; Notify charge status
        ret = charge_status_inform(chg_status);
        if (ret < 0)
        {
    ...
    // 计算时间差; Calculate time delta
    delta = (float)k_uptime_delta(&ref_time) / 1000.0f;

	// 返回值：返回计算出的电池荷电状态（SoC）[%]。; Returns the calculated battery state of charge (SoC) [%].
    soc = nrf_fuel_gauge_process(voltage, current, temp, delta, NULL);  

	// 获取电池在当前放电条件下预计还能使用的时间; Get the estimated time to empty (TTE) of the battery under the current discharge conditions
    tte = nrf_fuel_gauge_tte_get();     

	// 获取电池在当前充电条件下预计还需多长时间才能充满; Get the estimated time to full (TTF) of the battery under the current charge conditions
...
int pm1300_init(void)
{
    ...
    int ret = sensor_attr_get(charger, SENSOR_CHAN_CURRENT, SENSOR_ATTR_UPPER_THRESH, &val);// 读取vbus连接状态; Read vbus connection status
    if (ret < 0)
    {
        BSP_LOGI(TAG, "sensor_attr_get err[%d]!!!", ret);
        return -1;
    }
    vbus_connected = (val.val1 != 0) || (val.val2 != 0);// 读取vbus连接状态; Read vbus connection status
    fuel_gauge_update(charger, vbus_connected);// 初始更新; Initial update
...
/**
 * @brief 电池监测任务; Battery monitoring task
 * @note 该任务周期性地更新电池状态，包括电压、电流、温度等参数; This task periodically updates the battery status, including voltage, current, temperature, etc.
 */
void batter_monitor(void)
{
    // 更新充电器状态和Vbus连接状态; Update charger status and Vbus connection status
	if (!device_is_ready(charger))
	{
		BSP_LOGE(TAG, "Charger device not ready");
		return;
