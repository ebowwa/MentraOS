...
object STTTools {
    ...
    fun validateSTTModel(path: String): Boolean {
        try {
            ...
            val allTransducerFilesPresent = transducerFiles.all { fileName ->
                val file = File(modelDir, fileName)
                val exists = file.exists() && file.canRead() && file.length() > 0
                if (!exists) {
                    Bridge.log("STT model missing or invalid transducer file: $fileName at $path")
                }
                exists
            }
    ...
    fun extractTarBz2(sourcePath: String, destinationPath: String): Boolean {
        try {
            ...
            if (!destDir.exists()) {
                ...
                if (!destDir.mkdirs()) {
                    Bridge.log("EXTRACTION_ERROR: Failed to create destination directory: $destinationPath")
                    return false
                }
            }
                    ...
                    BZip2CompressorInputStream(bis).use { bzIn ->
                        ...
                        while (entry != null) {
                            try {
                                ...
                                Bridge.log("Processing entry: ${entry.name} (${entry.size} bytes, isDir=${entry.isDirectory})")
                                ...
                                if (entry.isDirectory) {
                                    ...
                                    FileOutputStream(outputFile).use { fos ->
                                        BufferedOutputStream(fos).use { bos ->
                                            val buffer = ByteArray(4096) // Use 4KB buffer like original implementation
                                            var len: Int
                                            var fileBytes = 0L
                                            ...
                                            while (tarIn.read(buffer).also { len = it } != -1) {
                                                ...
                                                // For large files (>10MB), log progress every 10MB
                                                if (fileSizeMB > 10) {
                                                    val currentMB = fileBytes / 1024 / 1024
                                                    if (currentMB >= lastProgressMB + 10) {
                                                        lastProgressMB = currentMB
                                                        val percent = if (entry.size > 0) (fileBytes * 100 / entry.size) else 0
                                                        Bridge.log("  Extracting ${entry.name}: ${currentMB}MB / ${fileSizeMB}MB (${percent}%)")
                                                    }
                                                }
                                            }
                                            ...
                                            Bridge.log("Extracted file $fileCount (${mbExtracted}MB total) - ${entry.name}")
                                        }
                                    }
                                }
                                ...
                                Bridge.log("Next entry received: ${entry?.name ?: "null (end of archive)"}")
                            } catch (e: Exception) {
                                Bridge.log("ERROR extracting entry ${entry?.name}: ${e.javaClass.simpleName}: ${e.message}")
                                e.printStackTrace()
                                throw e
                            }
                        }
            ...
            val mbExtracted = bytesExtracted / 1024 / 1024
            Bridge.log("Extraction completed successfully: $fileCount files, ${mbExtracted}MB in ${duration}s")
