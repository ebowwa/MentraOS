...
export class AuthService {
  ...
  async createTestAccounts(
    ...
    try {
      const credentials = this.accountService.createTestAccounts(
        emailTemplate,
        count,
      );
      console.log(
        `[AuthService] Created ${credentials.length}/${count} test accounts`,
      );
      return credentials;
    } catch (error) {
  ...
  async cleanupTestAccounts(emails: string[]): Promise<void> {
    console.log(
      "[AuthService] No cleanup needed - accounts are auto-created on connection",
    );
