...
export class StressTestRunner {
  ...
  static async spawnClients(
      ...
      const connectPromise = new Promise<void>((resolve, reject) => {
        setTimeout(async () => {
          try {
            ...
            console.error(
              `[StressTest] Client ${i + 1} failed to connect:`,
              error,
            );
            reject(error);
          }
    ...
    console.log(
      `[StressTest] ${successful}/${count} clients connected successfully`,
    );
  ...
  static async coordinatedTest(
    ...
    console.log(
      `[StressTest] Duration: ${options.duration}ms, Clients: ${clients.length}`,
    );

    // Count successful connections
    result.successfulConnections = clients.filter((c) =>
      c.isConnected(),
    ).length;
    ...
    result.metrics.connectionsPerSecond =
      result.successfulConnections / (testDuration / 1000);
    result.metrics.actionsPerSecond =
      result.successfulActions / (testDuration / 1000);
    ...
    console.log(
      `[StressTest] Success rate: ${result.successfulActions}/${result.totalActions} actions`,
    );
  ...
  static async runStressTest(
    ...
    console.log(
      `[StressTest] Starting stress test with ${config.clientCount} clients`,
    );
  ...
  private static async executeClientActions(
    ...
    for (const action of actions) {
      ...
      try {
        ...
        console.error(
          `[StressTest] Client ${clientIndex} action failed:`,
          action.type,
          error,
        );
        throw error;
  ...
  private static async disconnectClients(
    clients: MentraClient[],
  ): Promise<void> {
    const disconnectPromises = clients.map(async (client, index) => {
      try {
        ...
        console.error(
          `[StressTest] Failed to disconnect client ${index}:`,
          error,
        );
      }
    });
