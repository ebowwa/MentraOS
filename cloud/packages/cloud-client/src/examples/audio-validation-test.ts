  ...
  try {
    ...
    if (fmtIndex !== -1) {
      ...
      if (sampleRate !== expectedSampleRate) {
        console.warn(
          `⚠️  WARNING: Sample rate (${sampleRate}Hz) differs from expected (${expectedSampleRate}Hz)`,
        );
        console.log(
          "   This audio may need resampling for optimal transcription",
        );
      } else {
      ...
      if (bitsPerSample !== expectedBits) {
        console.warn(
          `⚠️  WARNING: Bit depth (${bitsPerSample}-bit) differs from expected (${expectedBits}-bit)`,
        );
      } else {
    ...
    console.log(
      `⏰ Estimated duration: ${durationSeconds.toFixed(1)} seconds (${(durationSeconds / 60).toFixed(1)} minutes)`,
    );
    ...
    console.log(
      `   Expected streaming time: ${((totalChunks * msPerChunk) / 1000).toFixed(1)} seconds`,
    );
