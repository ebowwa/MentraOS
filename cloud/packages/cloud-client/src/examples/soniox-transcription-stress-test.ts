...
async function main() {
  ...
  console.log(
    `📊 Testing with ${NUM_ACCOUNTS} accounts in batches of ${BATCH_SIZE}`,
  );
  ...
  const accounts = accountService.createTestAccounts(
    `soniox-test-user-{id}@test.com`,
    NUM_ACCOUNTS,
  );
  ...
  try {
    ...
    for (
      ...
      console.log(
        `🚀 Starting batch ${Math.floor(batchStart / BATCH_SIZE) + 1}: accounts ${batchStart + 1}-${batchEnd}`,
      );
    ...
    const timeoutPromise = new Promise<void>((_, reject) => {
      setTimeout(
        () => reject(new Error("Test timeout after 15 minutes")),
        900000,
      );
    });

    await Promise.race([Promise.allSettled(testPromises), timeoutPromise]);
...
function setupTranscriptionMonitoring(
  ...
  client.on("display_event", (event) => {
    ...
    if (
      ...
      if (overlap.length > 5) {
        // More than 5 characters overlap
        qualityData.overlappingSegments.push(
    ...
    if (qualityData.transcriptionHistory.length >= 2) {
      const current =
        qualityData.transcriptionHistory[
          qualityData.transcriptionHistory.length - 1
        ];
      const previous =
        qualityData.transcriptionHistory[
          qualityData.transcriptionHistory.length - 2
        ];

      if (
        Math.abs(current.timestamp - previous.timestamp) < 500 &&
        current.text !== previous.text &&
        current.text.length > 3 &&
        previous.text.length > 3
      ) {
...
function analyzeTranscriptionQuality(
  ...
  if (qualityData.finalCount === 0 && qualityData.interimCount > 0) {
    ...
    console.log(
      `⚠️  ${accountId}: No final transcriptions received (missing endpoint detection)`,
    );
  }

  // Log quality summary for this account
  if (
    qualityData.duplicateSegments.length > 0 ||
    qualityData.overlappingSegments.length > 0
  ) {
    console.log(
      `🐛 ${accountId}: Quality issues detected - Duplicates: ${qualityData.duplicateSegments.length}, Overlaps: ${qualityData.overlappingSegments.length}`,
    );
  } else {
...
function printTestResults(): void {
  ...
  console.log(
    `Successful Connections: ${stats.successfulConnections} (${((stats.successfulConnections / stats.totalAccounts) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Failed Connections: ${stats.failedConnections} (${((stats.failedConnections / stats.totalAccounts) * 100).toFixed(1)}%)`,
  );

  console.log("\n📱 APP MANAGEMENT STATISTICS:");
  console.log(
    `Successful App Starts: ${stats.successfulAppStarts} (${((stats.successfulAppStarts / stats.successfulConnections) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Failed App Starts: ${stats.failedAppStarts} (${((stats.failedAppStarts / stats.successfulConnections) * 100).toFixed(1)}%)`,
  );

  console.log("\n🎵 AUDIO STREAMING STATISTICS:");
  console.log(
    `Successful Audio Streams: ${stats.successfulAudioStreams} (${((stats.successfulAudioStreams / stats.successfulAppStarts) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Failed Audio Streams: ${stats.failedAudioStreams} (${((stats.failedAudioStreams / stats.successfulAppStarts) * 100).toFixed(1)}%)`,
  );
  ...
  console.log(
    `Total Duplicate Segments: ${stats.transcriptionIssues.duplicates}`,
  );
  console.log(
    `Total Overlapping Segments: ${stats.transcriptionIssues.overlaps}`,
  );
  console.log(
    `Total Incomplete Words: ${stats.transcriptionIssues.incompleteWords}`,
  );
  console.log(
    `Missing Endpoint Detection: ${stats.transcriptionIssues.missingEndTokens}`,
  );

  // Calculate quality metrics
  const totalTranscriptions = stats.qualityMetrics.reduce(
    (sum, q) => sum + q.interimCount + q.finalCount,
    0,
  );
  const totalIssues =
    stats.transcriptionIssues.duplicates +
  ...
  const qualityScore =
    totalTranscriptions > 0
      ? ((totalTranscriptions - totalIssues) / totalTranscriptions) * 100
      : 0;
  ...
  if (qualityScore >= 95) {
    console.log(
      "🎉 EXCELLENT: Soniox token processing improvements are working perfectly!",
    );
  } else if (qualityScore >= 80) {
    console.log(
      "✅ GOOD: Soniox improvements are working well with minor issues",
    );
  ...
  if (totalIssues > 0) {
    ...
    stats.qualityMetrics.slice(0, 5).forEach((quality) => {
      if (quality.duplicateSegments.length > 0) {
        console.log(
          `${quality.accountId} - Duplicates: ${quality.duplicateSegments.slice(0, 2).join(", ")}`,
        );
      }
      if (quality.overlappingSegments.length > 0) {
        console.log(
          `${quality.accountId} - Overlaps: ${quality.overlappingSegments.slice(0, 2).join(", ")}`,
        );
      }
      if (quality.corrections.length > 0) {
        console.log(
          `${quality.accountId} - Corrections: ${quality.corrections.slice(0, 2).join(", ")}`,
        );
      }
    });
  }
