...
async function main() {
  ...
  const client = new MentraClient({
    ...
    serverUrl:
      process.env.DEFAULT_SERVER_URL || "wss://testapi.mentra.glass/glasses-ws",
    debug: {
      logLevel: "info",
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "⚠️  Live Captions app unavailable, testing direct audio stream...\n",
        );
...
function setupTranscriptionMonitoring(client: MentraClient) {
  ...
  client.on("data_stream", (event) => {
    if (event.streamType === "transcription") {
      console.log(
        `🎙️  Direct Transcription: "${event.data.text}" (${event.data.isFinal ? "FINAL" : "interim"})`,
      );
...
function recordTranscription(event: TranscriptionEvent) {
  ...
  if (transcriptionHistory.length >= 2) {
    ...
    if (
      ...
      Math.abs(current.timestamp - previous.timestamp) < 2000
    ) {
      duplicateCount++;
    ...
    if (current.text.length > 3 && previous.text.length > 3) {
      ...
      if (overlap.length > 3 && current.text !== previous.text) {
        overlapCount++;
        console.log(
          `🚨 OVERLAP detected: "${previous.text}" → "${current.text}" (overlap: "${overlap}")`,
        );
      }
    }
    ...
    if (
      ...
      lastWord.length <= 3 &&
      !lastWord.match(
        /^(a|an|the|is|to|of|and|or|in|on|at|be|do|go|up|we|me|my|us|it|so|no|ok|yes)$/i,
      )
    ) {
      truncatedWordCount++;
      console.log(
        `🚨 TRUNCATED word detected: "${lastWord}" in "${current.text}"`,
      );
    }
  }
}
...
function analyzeTranscriptionQuality() {
  ...
  if (transcriptionHistory.length > 0) {
    ...
    transcriptionHistory.forEach((event, index) => {
      const timeOffset =
        index === 0 ? 0 : event.timestamp - transcriptionHistory[0].timestamp;
      console.log(
        `${String(index + 1).padStart(2)}) +${timeOffset}ms: "${event.text}"`,
      );
    });
  }
  ...
  const qualityScore =
    transcriptionHistory.length > 0
      ? ((transcriptionHistory.length - totalIssues) /
...
async function testDirectAudioStream(client: MentraClient) {
  console.log(
    "🎤 Testing direct audio streaming (without Live Captions app)...",
  );
  ...
  try {
    ...
    if (transcriptionHistory.length === 0) {
      console.log(
        "⚠️  No transcriptions received - may need Live Captions app",
      );
    } else {
      analyzeTranscriptionQuality();
    }
