...
function printTestResults(): void {
  ...
  console.log(
    `Circuit Breaker Triggered: ${stats.circuitBreakerTriggered ? "YES" : "NO"}`,
  );
  ...
  if (stats.circuitBreakerTriggered) {
    ...
    console.log(
      "ℹ️  Circuit breaker was not triggered - rate limiting may not have reached threshold",
    );
  }
  ...
  console.log(
    "✅ Stress test Azure transcription service with high concurrent load",
  );
  ...
  console.log(
    "✅ Verify improved retry logic with longer delays for rate limiting",
  );
