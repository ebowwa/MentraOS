...
export class AudioManager {
  ...
  async streamAudioFile(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(
          `[AudioManager] Streaming ${pcmData.length} bytes of PCM data`,
        );
        console.log(
          `[AudioManager] Chunk size: ${chunkSize} bytes, timing: ${msPerChunk.toFixed(1)}ms per chunk`,
        );
  ...
  private async streamChunksWithTiming(
    ...
    while (offset < data.length && this.isStreaming) {
      ...
      if (chunkCount % 10 === 0) {
        console.log(
          `[AudioManager] Sent chunk ${chunkCount}, ${offset}/${data.length} bytes`,
        );
      }
  ...
  private extractPCMFromWav(wavData: Buffer): Buffer | null {
    try {
      ...
      if (wavData.toString("ascii", 0, 4) !== "RIFF") {
        console.error(
          "[AudioManager] Not a valid WAV file - missing RIFF header",
        );
        return null;
      }
      ...
      if (wavData.toString("ascii", 8, 12) !== "WAVE") {
        console.error(
          "[AudioManager] Not a valid WAV file - missing WAVE format",
        );
        return null;
      }
      ...
      if (fmtIndex !== -1) {
        ...
        console.log(
          `[AudioManager] WAV Format: ${channels} channels, ${sampleRate}Hz, ${bitsPerSample}-bit`,
        );

        if (sampleRate !== this.config.sampleRate) {
          console.warn(
            `[AudioManager] WARNING: WAV sample rate (${sampleRate}Hz) differs from config (${this.config.sampleRate}Hz)`,
          );
        }
      }
      ...
      console.log(
        `[AudioManager] Extracted ${pcmData.length} bytes of PCM data from WAV file`,
      );
      ...
      console.log(
        `[AudioManager] Actual duration: ${actualDurationSeconds.toFixed(1)} seconds (${(actualDurationSeconds / 60).toFixed(1)} minutes)`,
      );

      // Convert to our target format if needed
      const convertedPCM = this.convertAudioToTargetFormat(
        pcmData,
        actualSampleRate,
        actualChannels,
      );
      ...
      console.log(
        `[AudioManager] Target duration after conversion: ${targetDurationSeconds.toFixed(1)} seconds`,
      );
   ...
   */
  private convertAudioToTargetFormat(
    pcmData: Buffer,
    sourceSampleRate: number,
    sourceChannels: number,
  ): Buffer {
    console.log(
      `[AudioManager] Converting from ${sourceSampleRate}Hz ${sourceChannels}ch to ${this.config.sampleRate}Hz 1ch`,
    );
    ...
    if (sourceChannels === 2) {
      ...
      console.log(
        `[AudioManager] Converted stereo to mono: ${convertedData.length} bytes`,
      );
    }
    ...
    if (sourceSampleRate !== this.config.sampleRate) {
      ...
      console.log(
        `[AudioManager] Resampled ${sourceSampleRate}Hz to ${this.config.sampleRate}Hz: ${convertedData.length} bytes`,
      );
    }
