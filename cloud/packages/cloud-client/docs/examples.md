...
async function testDashboardSwitching() {
  ...
  client.on("display_event", (display) => {
    ...
    console.log(
      `Currently visible: ${client.getVisibleContent()?.content || "nothing"}`,
    );
  });
...
async function monitorDisplayContent() {
  ...
  displayHistory.forEach((event, index) => {
    console.log(
      `${index + 1}. ${event.timestamp.toISOString()} - ${event.view}: ${event.content}`,
    );
  });
...
async function simulateWalking() {
  ...
  const route = [
    ...
    { lat: 37.815, lng: -122.475 },
    { lat: 37.81, lng: -122.47 },
    { lat: 37.805, lng: -122.465 },
    { lat: 37.8, lng: -122.46 },
    ...
    _read() {
      ...
      for (let i = 0; i < chunkSize; i++) {
        const sample = Math.sin(
          (2 * Math.PI * this.frequency * this.samples) / this.sampleRate,
        );
...
async function testTranslationWorkflow() {
  ...
  client.on("display_event", (display) => {
    if (
      ...
      display.layout.content.includes("translation")
    ) {
