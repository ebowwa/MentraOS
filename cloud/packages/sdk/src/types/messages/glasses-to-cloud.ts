 ...
 */
export interface ImuData extends BaseMessage {
  type: "imu_data";
  accel: [number, number, number]; // Accelerometer [x, y, z] in m/s²
  gyro: [number, number, number]; // Gyroscope [x, y, z] in rad/s
  mag: [number, number, number]; // Magnetometer [x, y, z] in μT
  quat: [number, number, number, number]; // Quaternion [w, x, y, z]
  euler: [number, number, number]; // Euler angles [roll, pitch, yaw] in degrees
  timestamp: Date; // Timestamp as Date to match BaseMessage
}

/**
 * IMU gesture detection from glasses
 */
export interface ImuGesture extends BaseMessage {
  type: "imu_gesture";
  gesture: string; // "head_up", "head_down", "nod_yes", "shake_no"
  timestamp: Date; // Timestamp as Date to match BaseMessage
}
