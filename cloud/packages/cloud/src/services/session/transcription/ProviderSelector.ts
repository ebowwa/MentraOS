...
export class ProviderSelector {
  ...
  private getSuggestedAlternatives(subscription: ExtendedStreamType): string[] {
    ...
    if (languageInfo.type === "translation") {
      allCapabilities.forEach((cap) => {
        const targetLanguages = cap.translationPairs.get(
          languageInfo.transcribeLanguage,
        );
        if (targetLanguages) {
          targetLanguages.forEach((target) => {
            suggestions.push(
              `translation:${languageInfo.transcribeLanguage}-to-${target}`,
            );
          });
        }
      });
    ...
    for (const [type, provider] of this.providers) {
      ...
      stats[type] = {
        ...
        capabilities: {
          ...
          translationPairs:
            provider.getLanguageCapabilities().translationPairs.size,
        },
      };
    }
