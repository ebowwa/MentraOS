...
async function optionalClientAuth(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
async function requireUser(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    logger.error(
      error,
      `requireUser: Failed to findOrCreateUser for email: ${authReq.email}`,
    );
    logger.debug(
      { req },
      `requireUser: Failed to findOrCreateUser for email: ${authReq.email}`,
...
async function requireUserSession(
  ...
  try {
    ...
    if (!userSession) {
      logger.warn(
        `requireUserSession: No active session found for user: ${userReq.email}`,
      );
    ...
    logger.info(
      `requireUserSession: User session populated for ${userReq.email}`,
    );
    next();
  } catch (error) {
    logger.error(
      error,
      `requireUserSession: Failed to fetch session for user: ${userReq.email}`,
    );
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
async function optionalUserSession(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  const localLogger =
    maybeAuthReq.logger || logger.child({ userId: "anonymous" });

  try {
    const email = maybeAuthReq.email;
    if (!email) {
      localLogger.info(
        `optionalUserSession: Anonymous request, continuing without session`,
      );
      return next();
    }
    ...
    if (userSession) {
      ...
      localLogger.info(
        `optionalUserSession: User session populated for ${email}`,
      );
    } else {
      localLogger.info(
        `optionalUserSession: No session found for ${email}, continuing without session`,
      );
    }
    ...
    localLogger.error(
      error as any,
      `optionalUserSession: Failed to fetch session`,
    );
    return next();
  }
}
...
export const authWithRequiredSession = [
  clientAuth,
  requireUser,
  requireUserSession,
];
export const authWithOptionalSession = [
  clientAuth,
  requireUser,
  optionalUserSession,
];
export const optionalAuthWithOptionalSession = [
  optionalClientAuth,
  optionalUserSession,
];
