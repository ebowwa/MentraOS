...
async function testCloudflareConnection() {
  ...
  console.log(
    `🔑 API Token: ${apiToken.substring(0, 10)}...${apiToken.substring(apiToken.length - 5)}\n`,
  );

  // Test 1: Verify token permissions
  console.log("1️⃣ Testing token permissions...");
  try {
    const tokenVerifyResponse = await axios.get(
  ...
  try {
    ...
    if (streamResponse.data.success) {
      ...
      console.log(
        `   Total live inputs: ${streamResponse.data.result_info?.total_count || 0}`,
      );
    } else {
    ...
    if (error.response?.status === 403) {
      ...
      console.error(
        "   4. Save and update the CLOUDFLARE_API_TOKEN in your .env file",
      );
    }
  }
