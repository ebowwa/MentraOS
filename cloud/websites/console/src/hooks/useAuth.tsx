  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      if (data.session?.access_token && !error) {
        ...
        const hasInviteToken = new URLSearchParams(window.location.search).has(
          "token",
        );
        if (!hasInviteToken) {
          setTimeout(() => {
    ...
    try {
      ...
      if (data.session?.access_token) {
        try {
          ...
          if (
            response.data.email &&
            data.session?.user?.email !== response.data.email
          ) {
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session?.access_token) {
        ...
        try {
          ...
          const hasInviteToken = new URLSearchParams(
