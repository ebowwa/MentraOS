...
const appService = {
  ...
  getPublicApps: async (): Promise<AppI[]> => {
    try {
      const response = await axios.get<ApiResponse<AppI[]>>(`/api/apps/public`);
  ...
  getInstalledApps: async (): Promise<AppI[]> => {
    try {
      const response =
        await axios.get<ApiResponse<AppI[]>>(`/api/apps/installed`);
      return response.data.data;
    } catch (error) {
...
const userService = {
  ...
  getCurrentUser: async (): Promise<User | null> => {
    try {
      const response = await axios.get<ApiResponse<User>>(`/api/user/me`);
      return response.data.data;
    } catch (error) {
...
const authService = {
  ...
  exchangeTemporaryToken: async (
    tempToken: string,
    packageName: string,
  ): Promise<TempTokenExchangeResponse> => {
    try {
