...
appSession.events.onPhoneNotifications(data => {
  if (data.priority === "high") {
    ...
    appSession.layouts.showTextWall("New Notification\n" + notificationText)
  }
})
```
...
appSession.events.onGlassesBattery(data => {
  ...
  const batteryStatus = data.charging ? `${data.level}% (Charging)` : `${data.level}%`
...
appSession.events.onLocation(data => {
  ...
  fetchWeatherForLocation(data.lat, data.lng).then(weather => {
    appSession.layouts.showTextWall(
...
appSession.events.onCalendarEvent(data => {
  ...
  // Show upcoming event
  appSession.layouts.showTextWall("Upcoming Event\n" + eventInfo)
})
```
